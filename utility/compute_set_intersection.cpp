...
ui ComputeSetIntersection::LeapfrogSeek(const VertexID *src, ui begin, ui end, ui target) {
    int bound = 1, halfBound;
    ui offset = begin + bound;
    while (offset < end && src[offset] < target) {
        bound *= 2;
        offset = begin + bound;
    }
    halfBound = bound / 2;
...
#if SI == 0
    return BinarySearchForGallopingSearchAVX2(src, begin + halfBound, end, target);
#else
    return BinarySearch(src, begin + halfBound, end, target);
#endif
}

void ComputeSetIntersection::LeapfrogJoin(VertexID **arrays, ui *counts, ui num, VertexID *cn, ui &cn_count) {
    cn_count = 0;
    for (int i = 0; i < num; ++i) {
        if (counts[i] == 0)
            return;
    ...
    while (true) {
        ...
        if (x == xPrime) {
            ...
            iters[p] = LeapfrogSeek(arrays[p], iters[p], counts[p], xPrime);
        }
        if (iters[p] == counts[p]) break;
        xPrime = arrays[p][iters[p]];
        p = (++p) % num;
    }
    delete[] iters;
}
